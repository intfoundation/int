...
Script.prototype.getCoinbaseFlag = function getCoinbaseFlag() {
    return Script.getCoinbaseFlag(this.raw);
};

Script.getCoinbaseFlag = function getCoinbaseFlag(raw) {
    if (raw.length === 0)
        return null;
    let buf = raw.slice(2, 2+raw[1]);
    return buf;
}
